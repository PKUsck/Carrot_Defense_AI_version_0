...
void GameScene::updateGame(double dt)
{
    ...
    for (auto& t : m_towers) {
        auto attack = t->getAttack();
        if (attack.fired) {
            auto proj = std::make_unique<Projectile>(
                t->centerPos(), attack.targetPos, attack.damage,
                attack.splashRadius, attack.slowFactor, attack.slowDuration,
                attack.color, 600.0,
                attack.poisonDps, attack.poisonDuration, attack.chainCount);
...
void GameScene::spawnEnemy()
{
    ...
    m_enemies.push_back(std::make_unique<Enemy>(type, m_waypoints));
}

void GameScene::handleProjectileHit(Projectile& proj)
{
    std::vector<Enemy*> hitTargets;

    if (proj.splashRadius() > 0) {
        for (auto& e : m_enemies) {
            if (!e->isActive()) continue;
            QPointF d = e->pos() - proj.targetPos();
            double dist = std::sqrt(d.x()*d.x() + d.y()*d.y());
            if (dist <= proj.splashRadius()) {
                double falloff = 1.0 - (dist / proj.splashRadius()) * 0.5;
                e->takeDamage(proj.damage() * falloff);
                hitTargets.push_back(e.get());
            }
        }
    } else if (proj.chainCount() > 0) {
        // Chain lightning: hit closest, then chain to next closest, etc.
        Enemy* first = nullptr;
        ...
        for (auto& e : m_enemies) {
            ...
            QPointF d = e->pos() - proj.targetPos();
            double dist = std::sqrt(d.x()*d.x() + d.y()*d.y());
            if (dist < bestDist) { bestDist = dist; first = e.get(); }
        }
        if (first) {
            first->takeDamage(proj.damage());
            hitTargets.push_back(first);

            // Chain to nearby enemies
            QPointF lastPos = first->pos();
            double chainDmg = proj.damage() * 0.6;
            for (int c = 1; c < proj.chainCount(); ++c) {
                Enemy* next = nullptr;
                double bestD2 = 100.0 * 100.0; // chain range 100px
                for (auto& e : m_enemies) {
                    if (!e->isActive()) continue;
                    bool alreadyHit = false;
                    for (auto* h : hitTargets) if (h == e.get()) { alreadyHit = true; break; }
                    if (alreadyHit) continue;
                    QPointF d = e->pos() - lastPos;
                    double d2 = d.x()*d.x() + d.y()*d.y();
                    if (d2 < bestD2) { bestD2 = d2; next = e.get(); }
                }
                if (!next) break;
                next->takeDamage(chainDmg);
                hitTargets.push_back(next);
                lastPos = next->pos();
                chainDmg *= 0.5;
            }
        }
        ...
        for (auto& e : m_enemies) {
            ...
            QPointF d = e->pos() - proj.targetPos();
            double dist = std::sqrt(d.x()*d.x() + d.y()*d.y());
            if (dist < closestDist) { closestDist = dist; closest = e.get(); }
        }
        if (closest) {
            closest->takeDamage(proj.damage());
            hitTargets.push_back(closest);
        }
    }

    // Apply slow & poison to all hit targets
    for (auto* e : hitTargets) {
        if (proj.slowFactor() < 1.0)
            e->applySlow(proj.slowFactor(), proj.slowDuration());
        if (proj.poisonDps() > 0)
            e->applyPoison(proj.poisonDps(), proj.poisonDuration());
    }

    for (auto& e : m_enemies) {
        if (e->isDead() && e->hp() <= 0 && e->reward() > 0)
            m_gold += e->reward();
    }
}
...
void GameScene::placeTower(int gx, int gy)
{
    ...
    TowerStats stats = Tower::statsForType(m_selectedTower);
    if (m_gold < stats.cost) return;

    m_towers.push_back(std::make_unique<Tower>(m_selectedTower, gx, gy, m_cellSize, m_offsetX, m_offsetY));
    m_gold -= stats.cost;
    update();
}
